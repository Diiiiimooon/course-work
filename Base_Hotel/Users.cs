...
namespace Base_Hotel
{
    ...
    {
        string login = "", pass; // Переменная для обновления ключевого поля
        public Users()
        {
            InitializeComponent();
        ...
        private void add_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!Policy_password.IsValid(fifth.Text))
                    throw new Exception();
                DateTime newDate = DateTime.Now.AddDays(Policy_password.PasswordExpirationDays);
                NpgsqlCommand npgsqlC = new NpgsqlCommand($"INSERT INTO hotel_base.users (\"Surname\", \"Name\", \"Patronymic\", login, password, \"chGuests\", \"chNumbers\", \"chGuests_Numbers\", \"chPayment\", \"chTimeOfResidence\", \"chUsers\", \"acGuests\", \"acNumbers\", \"acGuests_Numbers\", \"acPayment\", \"acTimeOfResidence\", \"acUsers\", \"dateUp\") VALUES " +
                    $"(@p1, @p2, @p3, @p4, '{Authorization.Crypt(fifth.Text, Authorization.secretKey)}', {chGuests.Checked}," + $"{chNumbers.Checked}, {chGuests_Numbers.Checked}, {chPayment.Checked}, {chTimeOfResidence.Checked}, {chUsers.Checked}, '{acGuests.SelectedItem}', '{acNumbers.SelectedItem}', '{acGuests_Numbers.SelectedItem}', '{acPayment.SelectedItem}', '{acTimeOfResidence.SelectedItem}', '{acUsers.SelectedItem}',@p5)", Hotel.npgsql);
        ...
        private void edit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (fifth.Text != pass)
                {
                    ...
                    $"\"acGuests\" = '{acGuests.SelectedItem}', \"acNumbers\" = '{acNumbers.SelectedItem}', \"acGuests_Numbers\" = '{acGuests_Numbers.SelectedItem}', \"acPayment\" = '{acPayment.SelectedItem}', \"acTimeOfResidence\" = '{acTimeOfResidence.SelectedItem}', \"acUsers\" = '{acUsers.SelectedItem}',\"dateUp\" = @p5 WHERE login = @p4", Hotel.npgsql);
                        npgsqlC.Parameters.AddWithValue("p1", first.Text);
                        npgsqlC.Parameters.AddWithValue("p2", second.Text);
                        npgsqlC.Parameters.AddWithValue("p3", third.Text);
                        npgsqlC.Parameters.AddWithValue("p4", fourth.Text);
        ...
        private void install_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (Convert.ToUInt32(minLength.Text) > Convert.ToUInt32(maxLength.Text) || ((Convert.ToUInt32(maxLength.Text) >= 0) && (Convert.ToUInt32(maxLength.Text) <= 3)) || Convert.ToUInt32(daysPassword.Text) == 0)
                    throw new Exception();
                Policy_password.MinimumLength = Convert.ToUInt32(minLength.Text);
                Policy_password.MaximumLength = Convert.ToUInt32(maxLength.Text);
                Policy_password.RequiresUppercase = upCase.Checked;
                Policy_password.RequiresLowercase = lowCase.Checked;
                Policy_password.RequiresNumber = numberCase.Checked;
                Policy_password.RequiresSpecialCharacter = specCase.Checked;
                Policy_password.PasswordExpirationDays = Convert.ToUInt32(daysPassword.Text);
                using (StreamWriter streamWriter = new StreamWriter("policy_password.txt", false))
                {
                    streamWriter.WriteLine($"Минимальная длина пароля: {Policy_password.MinimumLength}");
                    streamWriter.WriteLine($"Максимальная длина пароля: {Policy_password.MaximumLength}");
                    streamWriter.WriteLine($"Верхний регистр: {Policy_password.RequiresUppercase}");
                    streamWriter.WriteLine($"Нижний регистр: {Policy_password.RequiresLowercase}");
                    streamWriter.WriteLine($"Наличие цифр: {Policy_password.RequiresNumber}");
                    streamWriter.WriteLine($"Наличие одного из спец.символов (!@#): {Policy_password.RequiresSpecialCharacter}");
                    streamWriter.WriteLine($"Продолжительность действия пароля в днях: {Policy_password.PasswordExpirationDays}");
                }
            }
            catch (Exception)
            {
